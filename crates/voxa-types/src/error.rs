...
/// Classifies a Voxa failure for callers that need stable error handling.
...
pub enum ErrorCategory {
    ...
    Cancelled,
    /// A dependency outside Voxa failed.
    External,
    /// Voxa encountered an unexpected internal failure.
    Internal,
}

/// Adds structured, non-display context to a [`VoxaError`].
...
#[error("error code must start with VOXA-, be 6 through 64 bytes, and contain only uppercase ASCII letters, digits, and hyphens")]
pub struct ErrorCodeError;

/// A structured Voxa error with a stable code and optional source error.
#[derive(Debug, thiserror::Error)]
#[error("[{code}] {message}")]
pub struct VoxaError {
...
/// The result type used by Voxa public APIs.
pub type Result<T> = std::result::Result<T, VoxaError>;

impl VoxaError {
    /// Creates an error whose stable code has been validated.
    ///
    /// Panics if `code` is not a valid stable Voxa code. Use [`Self::try_new`]
    ...
    pub fn new(
        ...
        Self::try_new(category, code, message).expect("Voxa error code must be valid")
    }
...
mod tests {
    ...
    fn error_exposes_its_source() {
        let error = VoxaError::new(ErrorCategory::External, "VOXA-EXT-001", "dependency failed")
